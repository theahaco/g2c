 ...
 */
function extractAuthDataFromCbor(data: Uint8Array): Uint8Array {
  let offset = 0;
  ...
  if (majorType !== 5) throw new Error("Expected CBOR map for attestationObject");
 ...
 */
function extractP256FromCoseKey(coseBytes: Uint8Array): Uint8Array {
  let offset = 0;
  ...
  if (majorType !== 5) throw new Error("Expected CBOR map for COSE key");
  const mapLen = readLength(additionalInfo);

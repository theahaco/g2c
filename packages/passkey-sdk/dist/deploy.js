 ...
 */
export function computeAccountAddress(factoryContractId, salt, networkPassphrase) {
    return StrKey.encodeContract(hash(xdr.HashIdPreimage.envelopeTypeContractId(new xdr.HashIdPreimageContractId({
 ...
 */
export async function lookupExistingAccount(config, salt) {
    const server = new rpc.Server(config.rpcUrl);
    const address = computeAccountAddress(config.factoryContractId, salt, config.networkPassphrase);
    try {
        await server.getContractData(address, xdr.ScVal.scvLedgerKeyContractInstance());
        return address;
    }
    catch {
        return null;
 ...
 */
export async function deploySmartAccount(config, bundlerKeypair, salt, publicKey) {
    const server = new rpc.Server(config.rpcUrl);
    const expectedAddress = computeAccountAddress(config.factoryContractId, salt, config.networkPassphrase);
